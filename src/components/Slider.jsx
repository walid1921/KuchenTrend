    ...
    <div className="overflow-hidden ">
      <div className="embla__viewport" ref={emblaRef}>
        <div className="flex gap-3">
          {steps.map((step) => (
            <div key={step.id} className="bg-white w-[300px] px-4 pb-6 rounded-md">
              <p className="text-primary text-center font-extrabold text-[40px] 2xl:text-[130px] mx-9">
                {step.number}
              </p>
              <p className="text-secondary text-center mb-3 text-[22px] font-bold ">
                {step.title}
              </p>
              <p className="text-secondary 2xl:text-[16px] text-[14px] text-center 2xl:leading-[40px] leading-[30px]">
                {step.description}
              </p>
            </div>
          ))}
